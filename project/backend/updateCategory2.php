...
if (!isset($data['id_category']) || !isset($data['description'])) {
...
$sql = "UPDATE Category SET description = ? WHERE id_category = ?";
...
$stmt->bind_param("si", $description, $id_category);

// Ejecutar la consulta y verificar si fue exitosa
if ($stmt->execute()) {
    echo json_encode(['message' => 'Descripción actualizada exitosamente']);
} else {
    echo json_encode(['error' => 'Error al actualizar la descripción', 'details' => $stmt->error]);
}
